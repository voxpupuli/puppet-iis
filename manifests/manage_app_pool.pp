...
define iis::manage_app_pool (
  ...
  if $apppoolrecyclelogging != undef {
    if (!empty($apppoolrecyclelogging)) {
      ...
      $fixedloggingstring           = "\"${loggingstring}\"" # @"Time,Requests" as literal - we put this into powershell array constructor in execs

      $processAppPoolRecycleLogging = true
    } else {
      $fixedloggingstring           = ''
      $processAppPoolRecycleLogging = true # caller provided empty array for multi-value enum, wants to clear it
    }
  } else {
