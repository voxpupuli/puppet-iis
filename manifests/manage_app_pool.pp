    ...
    if ($process_apppool_identity) {
      if ($identitystring == 'SpecificUser') {
        exec { "app pool identitytype - ${app_pool_name} - SPECIFICUSER - ${apppool_username}":
          command   => "[System.Reflection.Assembly]::LoadWithPartialName(\"Microsoft.Web.Administration\");Import-Module WebAdministration;\$iis = New-Object Microsoft.Web.Administration.ServerManager;iis:;\$pool = get-item IIS:\\AppPools\\${app_pool_name};\$pool.processModel.username = \"${apppool_username}\";\$pool.processModel.password = \"${apppool_userpw}\";\$pool.processModel.identityType = ${identityenum};\$pool | set-item;",
          provider  => powershell,
          unless    => "[System.Reflection.Assembly]::LoadWithPartialName(\"Microsoft.Web.Administration\");Import-Module WebAdministration;\$iis = New-Object Microsoft.Web.Administration.ServerManager;iis:;\$pool = get-item IIS:\\AppPools\\${app_pool_name};if(\$pool.processModel.identityType -ne \"${identitystring}\"){exit 1;}\
if(\$pool.processModel.userName -ne \"${apppool_username}\"){exit 1;}if(\$pool.processModel.password -ne \"${apppool_userpw}\"){exit 1;}exit 0;",
        ...
        exec { "app pool identitytype - ${app_pool_name} - ${identitystring}":
          command   => "[System.Reflection.Assembly]::LoadWithPartialName(\"Microsoft.Web.Administration\");Import-Module WebAdministration;\$iis = New-Object Microsoft.Web.Administration.ServerManager;iis:;\$pool = get-item IIS:\\AppPools\\${app_pool_name};\$pool.processModel.identityType = ${identityenum};\$pool | set-item;",
          provider  => powershell,
          unless    => "[System.Reflection.Assembly]::LoadWithPartialName(\"Microsoft.Web.Administration\");Import-Module WebAdministration;\$iis = New-Object Microsoft.Web.Administration.ServerManager;iis:;\$pool = get-item IIS:\\AppPools\\${app_pool_name};if(\$pool.processModel.identityType -eq \"${identitystring}\"){exit 0;}else{exit 1;}",
          require   => Exec["Create-${app_pool_name}"],
          logoutput => true,
        }
      }
