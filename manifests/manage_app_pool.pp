define iis::manage_app_pool($app_pool_name = $title, $enable_32_bit = false, $managed_runtime_version = 'v4.0', $managed_pipeline_mode = 0, $identity_type = 0, $ensure = 'present') {

  validate_bool($enable_32_bit)
  validate_re($managed_runtime_version, ['^(v2\.0|v4\.0)$'])
  #Integrated = 0 Classic = 1 
  validate_re($managed_pipeline_mode, ['^(0|1)$'])
  #ApplicationPoolIdentity = 0 LocalService = 1, NetworkService = 2
  validate_re($identity_type, ['^(0|1|2)$'])
